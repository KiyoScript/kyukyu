<title>Sub-category | C Ducs Catering Services System</title>
...
?>
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody id="users_data">
                    <tr>
                      <?php
                        $sql = mysqli_query($conn, "SELECT * FROM sub_category JOIN category ON sub_category.sub_cat_Id=category.cat_Id");
                        while ($row = mysqli_fetch_array($sql)) {
                      ?>
                        <td><?php echo $row['cat_name']; ?></td>
                        <td><?php echo $row['sub_cat_name']; ?></td>
                        <td><?php echo $row['sub_description']; ?></td>
                        <td><?php echo $row['sub_date_added']; ?></td>
                        <td>
                            <div class="dropdown dropleft">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                      <button type="button" class="btn btn-primary dropdown-item" data-toggle="modal" data-target="#update<?php echo $row['sub_category_Id']; ?>">Update</button>
                                      <button type="button" class="btn btn-primary dropdown-item" data-toggle="modal" data-target="#delete<?php echo $row['sub_category_Id']; ?>">Delete</button>
                                </div>
                            </div>
                        </td>
                    </tr>
 ...
 <?php include 'sub_category_add.php'; ?>
